...
assert.match(telemetryRoutes, /sha256 = hashlib\.sha256/);
assert.match(telemetryRoutes, /link_points/);
...
console.log('PASS: ClaimTrace static + evidence engine + intake + multi-case + live telemetry + synchronized video artifact + persisted frame references + raw telemetry import + provenance + migration checks');
